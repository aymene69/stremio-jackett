...
# TODO: move this to a separate file where the cache get function is also located
...
def parse_xml(xml_content, media, config):
    ...
    for item in root.findall('.//item'):
        ...
        if media.type == "series":
            if config['language'] == "ru":
                if "S" + str(int(media.season.replace("S",""))) + "E" + str(int(media.episode.replace("E", ""))) not in title:
    ...
    threading.Thread(target=cache_torrents, args=(sorted_items, media.type, config)).start()
    return data
